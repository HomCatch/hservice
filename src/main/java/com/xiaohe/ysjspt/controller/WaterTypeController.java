...
public class WaterTypeController {
     ...
     */
    @PostMapping(value = "/")
    public Result insertCoupon(@Validated @RequestBody WaterType waterType) {
     ...
     */
    @PutMapping(value = "/{id}")
    public Result updateCoupon(@PathVariable Integer id, @RequestBody WaterType waterType){
        waterType.setId(id);
        Result result = null;
        try{
            result=insertCoupon(waterType);
        }catch (Exception e){
            log.error(e.getMessage());
